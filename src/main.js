...
class Game {
  ...
  _fireBeamTick() {
    ...
    if (bird.isGolden) earnedValue = Math.round(earnedValue * (bird.goldMult || 8));
    let isHeadshot = false;
    if (point && bird.mesh) {
      const bb = new THREE.Box3().setFromObject(bird.mesh);
      if (isFinite(bb.min.y) && bb.max.y > bb.min.y) {
        const frac = (point.y - bb.min.y) / (bb.max.y - bb.min.y);
        if (frac >= 0.62) isHeadshot = true;
      }
    }
    if (isHeadshot) earnedValue = Math.round(earnedValue * 1.5);
    ...
    if (isHeadshot) this.hud.showHeadshot();
    if (bird.isGolden) this.hud.showGolden(earnedValue);
  ...
  _killJuice() {
    ...
    if (bird.isGolden) earnedValue = Math.round(earnedValue * (bird.goldMult || 8));
    let isHeadshot = false;
    if (hitPoint && bird.mesh) {
      const bb = new THREE.Box3().setFromObject(bird.mesh);
      if (isFinite(bb.min.y) && bb.max.y > bb.min.y) {
        const frac = (hitPoint.y - bb.min.y) / (bb.max.y - bb.min.y);
        if (frac >= 0.62) isHeadshot = true;
      }
    }
    if (isHeadshot) earnedValue = Math.round(earnedValue * 1.5);
  ...
  _updateHunt(dt) {
    ...
    if (this.spawnTimer >= this.spawnInterval && this.birds.getLivingCount() < this.maxActiveBirds && !this._paradise) {
      ...
      if (!isBossSpawn && Math.random() < 0.04) {
        this.birds.spawnGolden(birdKey);
