...
export class UI {
  ...
  _bindButtons() {
    ...
    bind('btn-coop', () => { this.audio.playUIClick(); this._openCoopInvite(); });
  ...
  _coopOverlay(innerHTML) {
    const back = document.createElement('div');
    back.style.cssText = 'position:fixed;inset:0;background:rgba(20,12,2,.74);display:flex;align-items:center;justify-content:center;z-index:10001;';
    const box = document.createElement('div');
    box.style.cssText = 'max-width:460px;width:90%;background:linear-gradient(180deg,#2a2008,#1a1406);border:2px solid #8a6d2a;border-radius:16px;padding:24px;box-shadow:0 18px 60px rgba(0,0,0,.6);font-family:system-ui,sans-serif;color:#e8dcc0;';
    box.innerHTML = innerHTML;
    back.appendChild(box);
    document.body.appendChild(box.parentNode ? box.parentNode : back);
  ...
  async _openCoopInvite() {
    ...
    const startInvite = async (name) => {
      ...
      const poll = async () => {
        ...
        if (st && (st.status === 'declined' || st.status === 'cancelled')) { msg.style.color = '#ff8a8a'; msg.textContent = res.toName + ' is not available right now.'; waiting = false; return; }
  ...
  _startCoopRequestPolling() {
    if (this._coopReqTimer) return;
    const tick = async () => {
      try {
        if (this.economy && this.economy.uid && !this._coopReqOpen && !this.economy.coopSessionId) {
          const reqs = await this.economy.getIncomingHuntRequests();
          if (reqs && reqs.length) this._showIncomingCoopRequest(reqs[0]);
        }
      } catch (e) {}
      this._coopReqTimer = setTimeout(tick, 4000);
    };
    this._coopReqTimer = setTimeout(tick, 600);
  }

  _stopCoopRequestPolling() { if (this._coopReqTimer) { clearTimeout(this._coopReqTimer); this._coopReqTimer = null; } }
  ...
  showCoopWaiting(partnerName, onCancel) {
    if (this._coopWaitClose) { this._coopWaitClose(); this._coopWaitClose = null; }
    const o = this._coopOverlay(
      '<div style="font:900 21px system-ui;color:#ffd766;">Hunt Submitted!</div>' +
      '<div style="font:600 14px system-ui;color:#cbb98f;margin:10px 0 16px;">Waiting for <b style="color:#fff;">' + (partnerName || 'your friend') + '</b> to finish their hunt. You will both get the combined cash.</div>' +
      '<button id="coop-skip" style="width:100%;padding:10px;border:none;border-radius:10px;background:#3a2c10;color:#cbb98f;font:700 13px system-ui;cursor:pointer;">Keep my earnings and skip</button>'
    );
    this._coopWaitClose = o.close;
    o.box.querySelector('#coop-skip').addEventListener('click', () => { if (onCancel) onCancel(); o.close(); this._coopWaitClose = null; });
  }

  showCoopResults(r) {
    if (this._coopWaitClose) { this._coopWaitClose(); this._coopWaitClose = null; }
    const mvpLine = r.tie ? 'Tie game \u2014 you both earned the MVP bonus!' : (r.iAmMvp ? 'You are the MVP!' : (r.mvpName || 'Your friend') + ' was MVP.');
    const o = this._coopOverlay(
      '<div style="font:900 23px system-ui;color:#ffd766;text-shadow:0 0 10px rgba(255,200,80,.4);">Co-op Complete!</div>' +
      '<div style="font:600 14px system-ui;color:#cbb98f;margin:10px 0 14px;">You and <b style="color:#fff;">' + (r.partnerName || 'your friend') + '</b> hunted together.</div>' +
      '<div style="display:flex;gap:10px;margin-bottom:12px;">' +
        '<div style="flex:1;background:#16110a;border:1px solid #6b5630;border-radius:10px;padding:10px;text-align:center;"><div style="font:700 11px system-ui;color:#cbb98f;">YOU EARNED</div><div style="font:900 16px system-ui;color:#9fe0a0;">$' + (r.myMoney || 0).toLocaleString() + '</div></div>' +
  ...
  _playQuickDraw() {
    ...
    const loop = (now) => { if (!running) return; const dt = (now - last) / 1000; last = now; flash = Math.max(0, flash - dt);
      ...
      if (go) {
        ...
        ctx.fillStyle = '#fff'; ctx.font = '900 60px system-ui'; ctx.fillText('DRAW!', W / 2, H / 2 - 104);
      } else {
        ctx.fillStyle = '#e6d4a6'; ctx.font = '800 26px system-ui'; ctx.fillText(msg, W / 2, H / 2 - 6);
        ctx.fillStyle = '#9c8a5f'; ctx.font = '600 15px system-ui'; ctx.fillText('Wait for DRAW, then tap or press SPACE as fast as you can', W / 2, H / 2 + 30);
        const p = (Math.sin(now / 180) + 1) / 2; ctx.fillStyle = 'rgba(255,120,90,' + (0.3 + p * 0.5) + ')'; ctx.beginPath(); ctx.arc(W / 2, H / 2 + 72, 9, 0, 7); ctx.fill();
      }
  ...
  _playDodge() {
    const o = this._miniOverlay(); const ctx = o.canvas.getContext('2d'); const W = o.canvas.width, H = o.canvas.height;
    const groundY = H - 28;
    let score = 0, hits = 0, px = W / 2, keyx = 0, face = 1, walk = 0, poos = [], umbs = [], birds = [], puffs = [], spawnU = 0.5, spawnB = 0.6, last = performance.now(), raf = 0, running = true, t = 0, invuln = 0;
    const PW = 15;
    ...
    const onKey = (e) => { const k = e.key; if (k === 'ArrowLeft' || k === 'a' || k === 'A') { keyx = -1; e.preventDefault(); } else if (k === 'ArrowRight' || k === 'd' || k === 'D') { keyx = 1; e.preventDefault(); } };
    const onKeyUp = (e) => { const k = e.key; if ((k === 'ArrowLeft' || k === 'a' || k === 'A') && keyx === -1) keyx = 0; else if ((k === 'ArrowRight' || k === 'd' || k === 'D') && keyx === 1) keyx = 0; };
    ...
    const drawUmb = (u) => { ctx.save(); ctx.translate(u.x, groundY + Math.sin(t * 3 + u.ph) * 2); ctx.strokeStyle = '#7a4d1c'; ctx.lineWidth = 2.5; ctx.beginPath(); ctx.moveTo(0, 0); ctx.lineTo(0, -22); ctx.stroke(); ctx.beginPath(); ctx.arc(3, 0, 3, Math.PI, 6.28); ctx.stroke(); ctx.translate(0, -22); const cols = u.gold ? ['#ffd24a', '#e0a91e'] : ['#e0463b', '#f4f4f4']; for (let i = 0; i < 6; i++) { ctx.fillStyle = cols[i % 2]; const a0 = Math.PI + i / 6 * Math.PI, a1 = Math.PI + (i + 1) / 6 * Math.PI; ctx.beginPath(); ctx.moveTo(0, 0); ctx.arc(0, 0, 15, a0, a1); ctx.closePath(); ctx.fill(); } ctx.strokeStyle = 'rgba(0,0,0,0.25)'; ctx.lineWidth = 1; ctx.beginPath(); ctx.arc(0, 0, 15, Math.PI, 0); ctx.stroke(); ctx.fillStyle = u.gold ? '#e0a91e' : '#b83228'; ctx.beginPath(); ctx.arc(0, -15, 2, 0, 7); ctx.fill(); ctx.restore(); };
    ...
    const drawPerson = (x, hurt) => { ctx.save(); ctx.translate(x, groundY); const lp = Math.sin(walk) * 5; ctx.strokeStyle = '#26324a'; ctx.lineWidth = 5; ctx.lineCap = 'round'; ctx.beginPath(); ctx.moveTo(0, -15); ctx.lineTo(-4 + lp, 0); ctx.moveTo(0, -15); ctx.lineTo(4 - lp, 0); ctx.stroke(); ctx.fillStyle = hurt ? '#c0392b' : '#3f7fd0'; ctx.fillRect(-8, -37, 16, 23); ctx.strokeStyle = hurt ? '#c0392b' : '#3f7fd0'; ctx.lineWidth = 4; ctx.beginPath(); ctx.moveTo(-7, -33); ctx.lineTo(-13, -22 + lp); ctx.moveTo(7, -33); ctx.lineTo(13, -22 - lp); ctx.stroke(); ctx.fillStyle = '#f0c9a0'; ctx.beginPath(); ctx.arc(0, -45, 8, 0, 7); ctx.fill(); ctx.fillStyle = '#3a2a1a'; ctx.beginPath(); ctx.arc(0, -47, 8, Math.PI, 0); ctx.fill(); ctx.fillStyle = '#000'; ctx.beginPath(); ctx.arc(face * 3, -45, 1.4, 0, 7); ctx.fill(); ctx.restore(); };
    const loop = (now) => { if (!running) return; const dt = Math.min(0.05, (now - last) / 1000); last = now; t += dt; invuln = Math.max(0, invuln - dt);
      ...
      for (const p of poos) { if (p.dead) continue; p.y += p.vy * dt; p.rot += p.vr * dt; drawPoo(p); if (invuln <= 0 && Math.abs(p.x - px) < PW + 6 && p.y > groundY - 42 && p.y < groundY) { p.dead = true; hits++; invuln = 1.3; for (let i = 0; i < 12; i++) puffs.push({ x: p.x, y: p.y, vx: (Math.random() - 0.5) * 200, vy: -Math.random() * 120, life: 0.55, col: '#6b4a22' }); if (hits >= 2) { cleanup(); this._miniFinish(o, 'Dodge the Dookie', score); return; } } else if (p.y >= groundY) { p.dead = true; for (let i = 0; i < 5; i++) puffs.push({ x: p.x, y: groundY, vx: (Math.random() - 0.5) * 90, vy: -Math.random() * 40, life: 0.4, col: '#6b4a22' }); } }
      poos = poos.filter((p) => !p.dead);
      for (const pf of puffs) { pf.life -= dt; pf.x += pf.vx * dt; pf.y += pf.vy * dt; pf.vy += 320 * dt; ctx.globalAlpha = Math.max(0, pf.life * 2); ctx.fillStyle = pf.col; ctx.beginPath(); ctx.arc(pf.x, pf.y, 3, 0, 7); ctx.fill(); ctx.globalAlpha = 1; }
      puffs = puffs.filter((pf) => pf.life > 0);
      if (!(invuln > 0 && Math.floor(t * 12) % 2 === 0)) drawPerson(px, invuln > 0);
      ctx.fillStyle = '#c0392b'; for (let i = 0; i < 2; i++) { ctx.globalAlpha = i < (2 - hits) ? 1 : 0.22; const hx = 22 + i * 28, hy = 22; ctx.beginPath(); ctx.moveTo(hx, hy); ctx.bezierCurveTo(hx - 9, hy - 9, hx - 15, hy + 3, hx, hy + 14); ctx.bezierCurveTo(hx + 15, hy + 3, hx + 9, hy - 9, hx, hy); ctx.fill(); } ctx.globalAlpha = 1;
  ...
  showDailyReward(info) {
    if (!info) return;
    const back = document.createElement('div');
    back.style.cssText = 'position:fixed;inset:0;background:rgba(20,12,2,.72);display:flex;align-items:center;justify-content:center;z-index:10000;';
    const cycle = ((info.day - 1) % 7);
    const cells = DAILY_REWARDS.map((r, i) => {
      const on = i === cycle;
      const done = i < cycle;
      const lbl = r.label || (r.money ? ('$' + r.money) : (r.ticket + 'T'));
      return '<div style="flex:1;min-width:54px;padding:8px 4px;border-radius:8px;text-align:center;border:2px solid ' + (on ? '#ffd766' : '#6b5630') + ';background:' + (on ? 'linear-gradient(180deg,#5a4416,#3a2c0e)' : (done ? '#2a2110' : '#221a0c')) + ';opacity:' + (done && !on ? 0.55 : 1) + ';">' +
        '<div style="font:700 11px system-ui;color:#cbb98f;">Day ' + (i + 1) + '</div>' +
        '<div style="font:900 14px system-ui;color:' + (r.ticket ? '#c79bff' : '#ffd766') + ';margin-top:3px;">' + lbl + '</div>' +
        (on ? '<div style="font:800 10px system-ui;color:#9fe0a0;margin-top:3px;">TODAY</div>' : '') +
      '</div>';
    }).join('');
    const gain = [];
    if (info.money) gain.push('$' + info.money.toLocaleString());
    if (info.tickets) gain.push(info.tickets + ' Ticket' + (info.tickets > 1 ? 's' : ''));
    const box = document.createElement('div');
    box.style.cssText = 'max-width:560px;width:92%;background:linear-gradient(180deg,#2a2008,#1a1406);border:2px solid #8a6d2a;border-radius:16px;padding:22px;box-shadow:0 18px 60px rgba(0,0,0,.6);font-family:system-ui,sans-serif;';
    box.innerHTML = '<div style="font:900 22px system-ui;color:#ffd766;text-shadow:0 0 10px rgba(255,200,80,.4);">Daily Reward</div>' +
      '<div style="font:600 14px system-ui;color:#cbb98f;margin:4px 0 14px;">Day ' + info.day + ' streak &mdash; you earned <b style="color:#fff;">' + (gain.join(' + ') || 'a reward') + '</b></div>' +
      '<div style="display:flex;gap:6px;flex-wrap:wrap;">' + cells + '</div>' +
      '<button id="daily-collect" style="margin-top:16px;width:100%;padding:11px;border:none;border-radius:10px;background:linear-gradient(180deg,#5bbf5b,#2e9d3a);color:#fff;font:900 16px system-ui;cursor:pointer;">Collect</button>';
    back.appendChild(box);
    document.body.appendChild(back);
    const close = () => { if (back.parentNode) back.parentNode.removeChild(back); };
    const btn = box.querySelector('#daily-collect');
